...
public class Card {
    ...
    ///
    init(color: CardColor, shape: CardShape, filling: CardFilling, number: CardNumber) {
        self.color   = color;
        self.shape   = shape;
    ...
    static func matchingSetValue(_ b: Card, _ c: Card) -> Card {
        let color   : CardColor   = CardColor  .matchingSetValue(b.color,   c.color);
        let shape   : CardShape   = CardShape  .matchingSetValue(b.shape,   c.shape);
        let filling : CardFilling = CardFilling.matchingSetValue(b.filling, c.filling);
        let number  : CardNumber  = CardNumber .matchingSetValue(b.number,  c.number);
