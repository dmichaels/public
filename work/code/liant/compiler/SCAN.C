/* ANSI-C scan.c - Lexical scanning routines */
...
"@(#)scan.c  010  11/21/89  (c) 1989 Language Processors, Inc.";
...
SSCANC ()
{
	...
	for (lastc = EOF_CHAR, newline = 0 ; 1 ; lastc = c) {
		...
		if ((c = GETC (f)) == SPACE_CHAR)
			...
			if ((nextc = GETC (f)) == STAR_CHAR) {
				if ((c = scomment ()) != SPACE_CHAR)
					return (c);
			}
#ifdef C_PLUS_PLUS
			else if (SRC_CXX_COMMENTS () && (nextc == SLASH_CHAR)) {
				if ((c = scomment_cxx ()) != SPACE_CHAR)
					return (c);
			}
#endif
			else {	UNGETC (f, nextc);
...
**
** This routine is used to scan to the next relevant input character
** when the lexer is processing a preprocessing directive line.
**
** Returns from the current source file, one of:
**
**	Next relevant character
...
SSCANX ()
{
	...
	if (c == END_PP_DIRECTIVE_CHAR)
		...
		if (((c = SNEXTC ()) == LOWER_L_CHAR) || (c == LOWER_N_CHAR)) {
			UNGETC (SRC_FILE_POINTER (), c);
			UNGETC (SRC_FILE_POINTER (), LOWER_E_CHAR);
			SRC_END_EXCLUSION ();
			return (BEGIN_PP_DIRECTIVE_CHAR);
		}
	}
		...
		if ((c = SNEXTC ()) == LOWER_F_CHAR) {
			UNGETC (SRC_FILE_POINTER (), c);
			UNGETC (SRC_FILE_POINTER (), LOWER_I_CHAR);
			SRC_END_EXCLUSION ();
			return (BEGIN_PP_DIRECTIVE_CHAR);
		}
	}
...
** is pushed back.  Up to MAX_PUSH_BACK characters (defined in cfile.h)
** may be safely pushed back.
** ------------------------------------------------------------------- */
