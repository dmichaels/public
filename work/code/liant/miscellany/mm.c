...
instruct(){
 ...
 register int perms = 1, i;

 for(i = 0 ; i < MMSZ ; i++)
	perms *= NPEGS;

 printf("\nI will act as a passive partner in the game of master mind.\n\n");
 printf("     * I will create a code consisting of %d \"colored pegs\".\n"
                ,MMSZ);
 printf("       In this case, the \"colored peg\" will be\n");
 printf("       %d characters from the set [%s].\n",NPEGS,CodeChars);
 printf("     * You must make successive guesses at the code\n");
 printf("       and modify your guesses using the below clues\n");
 printf("       until you guess the code, or until you run out\n");
 printf("       of turns (%d). There are %d possible codes.\n"
                ,MMSZ,perms);
 printf("     * A \"black\" reply means that you\n");
 printf("       have a correct color in a correct position.\n");
 printf("     * A \"white\" reply means that you\n");
 printf("       have a correct color in an incorrect position.\n");
 printf("     * You have %d tries to guess the code.\n"
                ,MAXTRY);
